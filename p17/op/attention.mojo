    ...
    fn execute[
        ...
        if target == "gpu":
            ...
            gpu_ctx.enqueue_function[
                transpose_kernel[layout_k, layout_k_t, d, seq_len]
            ](
                k_t,
                k_tensor,
            ...
            scores_tensor_2d = LayoutTensor[
                mut=True, dtype, layout_scores_2d, MutableAnyOrigin
            ](scores_weights_buf.unsafe_ptr())
            gpu_ctx.enqueue_function[
                matmul_idiomatic_tiled[layout_scores_2d, 1, seq_len, d]
            ](
                scores_tensor_2d,
                q_2d,
                k_t,
            ...
            scores_tensor = scores_tensor_2d.reshape[layout_scores]()

            # Step 5: Apply softmax to get attention weights
            gpu_ctx.enqueue_function[softmax_kernel[layout_scores, seq_len]](
                scores_tensor,
                scores_tensor,
                grid_dim=(1, 1),
                block_dim=(seq_len, 1),
            ...
            reshaped_scores = scores_tensor.reshape[layout_scores_2d]()

            # Step 7: Compute final result using matmul: weights @ V = (1, seq_len) @ (seq_len, d) -> (1, d)
            # Reuse out_tensor reshaped as (1, d) for result
            final_result = out_tensor.reshape[layout_result_2d]()
            gpu_ctx.enqueue_function[
                matmul_idiomatic_tiled[layout_result_2d, 1, d, seq_len]
            ](
                final_result,
                reshaped_scores,
                v_tensor,
