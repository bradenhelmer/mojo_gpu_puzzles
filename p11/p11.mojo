...
fn conv_1d_simple[
    ...
    shared_kernel = tb[dtype]().row_major[CONV]().shared().alloc()
    if global_i < SIZE:
        shared_in[local_i] = a[global_i]
    if global_i < CONV:
        shared_kernel[local_i] = b[global_i]
    barrier()
    if global_i < SIZE:
        for j in range(CONV):
            if local_i + j < SIZE:
                out[global_i] += shared_in[local_i + j] * shared_kernel[j]
...
fn conv_1d_block_boundary[
    ...
    shared_kernel = tb[dtype]().row_major[CONV_2]().shared().alloc()
    if global_i < SIZE_2:
        shared_in[local_i] = a[global_i]
