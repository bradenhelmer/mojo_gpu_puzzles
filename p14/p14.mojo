...
fn matmul_tiled[
    layout: Layout, size: Int, manual: Bool
](
    out: LayoutTensor[mut=False, dtype, layout],
    a: LayoutTensor[mut=False, dtype, layout],
    b: LayoutTensor[mut=False, dtype, layout],
    ...
    if manual:
        ...
        for tile in range((size + TPB - 1) // TPB):
            if local_col < TPB and local_row < TPB:
        ...
        if (
            ...
            out_tile[local_row, local_col] = sum
...
def main():
    with DeviceContext() as ctx:
        size = SIZE_TILED if argv()[1].startswith("--tiled") else SIZE
        ...
        if argv()[1] == "--naive":
            ...
            if argv()[1] == "--tiled-manual":
                ctx.enqueue_function[
                    matmul_tiled[layout_tiled, SIZE_TILED, True]
                ](
                    out_tensor_tiled,
                    a_tensor_tiled,
                    b_tensor_tiled,
                ...
                ctx.enqueue_function[
                    matmul_tiled[layout_tiled, SIZE_TILED, False]
                ](
                    out_tensor_tiled,
                    a_tensor_tiled,
                    b_tensor_tiled,
